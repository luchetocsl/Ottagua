...
- **Plunk API** for newsletter subscriptions (requires `PUBLIC_PLUNK_API_KEY` in `.env`)
...
Required in `.env`:
- `PUBLIC_PLUNK_API_KEY` - For newsletter subscription functionality (must be prefixed with `PUBLIC_` to be accessible client-side)
